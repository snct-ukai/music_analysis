...
class Separator():
    ...
    def detect_segment(self):
        ...
        for i in range(0, len(self.verse_time) -1, 1):
            ...
            feat = calc_feat(data[start:end], sr)
            ...
            mean = valid_convolve(combined_score, window_size)
            std = valid_convolve((combined_score - mean)**2, window_size)**0.5
            n = 2.
            ...
            print(change_samples, "change_samples")
    ...
    def separate(self):
        ...
        for i, seg in enumerate(self.segment_time):
            ...
            sf.write(f'./output/{self.filename}/segment_{i}.wav', self.data[start:end], sr)
        print("Separation is done")

        import matplotlib.pyplot as plt
        plt.plot(self.data)
        for seg in self.segment_time:
            plt.axvline(seg * sr, color='r')
        for verse in self.verse_time:
            plt.axvline(verse * sr, color='g')
        plt.show()
